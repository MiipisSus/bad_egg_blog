...
const emptyForm: ActivityForm = { title: "", description: "", image: null }
...
export default function AdminActivitiesPage() {
  ...
  function openCreate() {
    setEditingId(null); setForm(emptyForm); setImagePreview(null); setShowModal(true)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      setMessage({ text: data.message, type: "success" }); setShowModal(false); fetchItems()
    } else {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const res = await fetch("/api/activities/sort", {
      method: "POST",
...
function SortableRow({ item, index, onEdit, onDelete }: { item: Activity; index: number; onEdit: (i: Activity) => void; onDelete: (id: number) => void }) {
  ...
  return (
    <tr ref={setNodeRef} style={style} className="border-b border-border last:border-0 bg-background">
      ...
      <td className="px-4 py-3 max-w-[200px] truncate text-muted-foreground">{item.description || "—"}</td>
